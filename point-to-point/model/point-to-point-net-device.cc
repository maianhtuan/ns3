...
static void m_TxPowerDownTrigger (PointToPointNetDevice *device); // Timer trigger : Tx down
static void m_RxPowerDownTrigger (PointToPointNetDevice *device); // Timer trigger : Rx down
...
void
PointToPointNetDevice::NIC__UP ()
{
  NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " NIC__UP");
  // Change state UP
  ChangeState (TX_UP);
  ChangeState (RX_UP);
  // Schedule UP -> READY
...
void
PointToPointNetDevice::NIC__DOWN ()
{
  NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " NIC__DOWN");
  // Change state DOWN
  ChangeState (TX_DOWN);
  ChangeState (RX_DOWN);
  // Schedule DOWN -> SLEEP
  Simulator::Schedule (Seconds (m_t_down), &m_TxPowerDownTrigger, this);
  Simulator::Schedule (Seconds (m_t_down), &m_RxPowerDownTrigger, this);
}

void
PointToPointNetDevice::NIC__TX__UP ()
{
	if (m_eee == false)
    {
      // Cancel TX__UP if EEE disabled
      return;
    }

  setTxLock(true);

	NS_LOG_FUNCTION (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " NIC__TX__UP");
...
void
PointToPointNetDevice::NIC__RX__UP ()
{
	if (m_eee == false)
    {
  ...
  setRxLock(true);
	...
	NS_LOG_FUNCTION (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " NIC__RX__UP");
...
void
PointToPointNetDevice::NIC__TX__DOWN ()
{
	if (m_eee == false)
    {
      // Cancel TX__DOWN if EEE disabled
      return;
    }

  setTxLock(false);

	NS_LOG_FUNCTION (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " NIC__TX__DOWN");

	if ((m_txMachineState == TX_READY) && (TxLock == false) && (RxLock == false))
		{
			// Sleep remote
      SendPCSMessage(PointToPointNetDevice::PCS_DOWN);
      // Sleep local
      NIC__DOWN ();
		}
}
...
void
PointToPointNetDevice::NIC__RX__DOWN ()
{
	if (m_eee == false)
    {
      // Cancel RX__DOWN if EEE disabled
      return;
    }

  setRxLock(false);

	NS_LOG_FUNCTION (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " NIC__RX__DOWN");

	if ((m_rxMachineState == RX_READY) && (TxLock == false) && (RxLock == false))
		{
			// Sleep local
      NIC__DOWN ();
		}
...
PointToPointNetDevice::m_TxWakeUp ()
{
    ...
    {
      NS_LOG_WARN (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " Expected TX_UP to wake up");
    }
}
...
PointToPointNetDevice::m_RxWakeUp ()
{
    ...
    {
      NS_LOG_WARN (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " Expected RX_UP to wake up");
    }
}
...
PointToPointNetDevice::m_TxPowerDown ()
{
    ...
    {
      NS_LOG_WARN (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " Expected TX_DOWN to sleep");
    }
}
...
PointToPointNetDevice::m_RxPowerDown ()
{
    ...
    {
      NS_LOG_WARN (m_address << "(" << TxLock << ", " << RxLock << ") [ ] " << Simulator::Now ().GetSeconds() << "\t" << " Expected RX_DOWN to sleep");
    }
			...
			if (m_ticks_count != -1)
				{
					...
					unsigned int queue_tresh = (unsigned int) ((double) ((double) m_queue_factor / (double) 100) * (double) m_queue_capacity.Get());

					if (m_ticks_count == 0)
						{
							// Timeout event
							event_timeout++;
							NS_LOG_DEBUG (m_address << "(" << TxLock << ", " << RxLock << ") [!] " << Simulator::Now ().GetSeconds() << "\t" << " (timeout - " << m_queue->GetNPackets () << "/" << queue_tresh << " packets - " << (m_ticks_max - m_ticks_count) << "/" << m_ticks_max << " ticks)");
							m_ticks_count = -1;
							NIC__TX__UP ();
						}
					else if ((m_queue->GetNPackets ()) >= queue_tresh)
						{
							// Max queue event
							event_queue++;
							NS_LOG_DEBUG (m_address << "(" << TxLock << ", " << RxLock << ") [!] " << Simulator::Now ().GetSeconds() << "\t" << " (full    - " << m_queue->GetNPackets () << "/" << queue_tresh << " packets - " << (m_ticks_max - m_ticks_count) << "/" << m_ticks_max << " ticks)");
							m_ticks_count = -1;
							NIC__TX__UP ();
						}
					else
...
static void
m_TxPowerDownTrigger (PointToPointNetDevice *device)
{
	NS_LOG_FUNCTION (device);
...
static void
m_RxPowerDownTrigger (PointToPointNetDevice *device)
{
	NS_LOG_FUNCTION (device);
	...
	switch (msg)
		{
				case PointToPointNetDevice::PCS_UP:
					{
						NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [P] " << Simulator::Now ().GetSeconds() << "\t" << " PCS Message received : WAKEUP");
						NIC__RX__UP ();
					}
					break;
				...
				case PointToPointNetDevice::PCS_DOWN:
					{
						NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [P] " << Simulator::Now ().GetSeconds() << "\t" << " PCS Message received : SLEEP");
						NIC__RX__DOWN ();
					}
					break;
				...
				default:
					{
						NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [P] " << Simulator::Now ().GetSeconds() << "\t" << " PCS Message received : ???");
					}
			...
			switch (newState)
				{
					case PointToPointNetDevice::RX_SLEEP:
						{
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "RX_SLEEP" << " (+" << energyConsumption << "J during " << duration << ")");
							//NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " ** Rx OFF **");
						}
						break;
					case PointToPointNetDevice::RX_READY:
						{
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "RX_READY" << " (+" << energyConsumption << "J during " << duration << ")");
						}
						break;
					case PointToPointNetDevice::RX_BUSY:
						{
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "RX_BUSY" << " (+" << energyConsumption << "J during " << duration << ")");
						}
						break;
					case PointToPointNetDevice::RX_UP:
						{
							//NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " ** Rx ON **");
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "RX_UP" << " (+" << energyConsumption << "J during " << duration << ")");
						}
						break;
					case PointToPointNetDevice::RX_DOWN:
						{
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "RX_DOWN" << " (+" << energyConsumption << "J during " << duration << ")");
						}
						break;
				}
		}
...
PointToPointNetDevice::ChangeState (TxMachineState newState)
{
	...
	if (newState == PointToPointNetDevice::TX_SLEEP)
		{
			Simulator::ScheduleNow (&m_TxSleepClockTrigger, this);
		}
			...
			switch (newState)
				{
					case PointToPointNetDevice::TX_SLEEP:
						{
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "TX_SLEEP" << " (+" << energyConsumption << "J during " << duration << ")");
							//NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " ** Tx OFF **");
						}
						break;
					case PointToPointNetDevice::TX_READY:
						{
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "TX_READY" << " (+" << energyConsumption << "J during " << duration << ")");
						}
						break;
					case PointToPointNetDevice::TX_BUSY:
						{
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "TX_BUSY" << " (+" << energyConsumption << "J during " << duration << ")");
						}
						break;
					case PointToPointNetDevice::TX_UP:
						{
							//NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " ** Tx ON **");
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "TX_UP" << " (+" << energyConsumption << "J during " << duration << ")");
						}
						break;
					case PointToPointNetDevice::TX_DOWN:
						{
							NS_LOG_INFO (m_address << "(" << TxLock << ", " << RxLock << ") [M] " << Simulator::Now ().GetSeconds() << "\t" << " Device mode changed to " << "TX_DOWN" << " (+" << energyConsumption << "J during " << duration << ")");
						}
						break;
				}
...
PointToPointNetDevice::Send (Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
	...
	if (m_queue->Enqueue (packet) == false)
		{
			NS_LOG_WARN (m_address << "(" << TxLock << ", " << RxLock << ") [!] " <<	Simulator::Now ().GetSeconds() << "\t" << " Warning: failed to add packet to queue");
			m_macTxDropTrace (packet);
			m_packetsDropped++;
			return false;
		}
	else
		{
			...
			NS_LOG_LOGIC (m_address << "(" << TxLock << ", " << RxLock << ") [+] " <<	Simulator::Now ().GetSeconds() << "\t\t" << " Packet added (" << m_queue->GetNPackets () << ")");

			// Set the maximum time
			if ((m_ticks_count == -1) && (m_queue->GetNPackets () == 1))
				{
					m_ticks_count = m_ticks_max;
				}

			// Check if we can start the transmission right now
...
PointToPointNetDevice::initTransmissionIfPossible ()
{
  if (m_txMachineState == TX_READY)
    {
      // The interface is UP, lock Tx
      setTxLock(true);
      
      if (GetRemoteNetDevice()->IsLocalReceiveOn() == false)
        {
          // Remote NetDevice is already in LPI mode, wake it up before starting transmission
          SendPCSMessage(PCS_UP);
          Simulator::Schedule (Seconds (m_t_up), &PointToPointNetDevice::initTransmission, this);
        }
      else
        {
          // Remote NetDevice is already active, start transmission
          initTransmission ();
        }
    }
}
...
PointToPointNetDevice::TransmitComplete (void)
{
  ...
  // If the queue is empty, we are done, otherwise we need to start transmitting the next packet.
	...
	NS_LOG_LOGIC (m_address << "(" << TxLock << ", " << RxLock << ") [-] " << Simulator::Now ().GetSeconds() << "\t\t" << " Packet removed (" << m_queue->GetNPackets() << ")");
	...
	if (p != 0)
		...
		{
			// No packet was on the queue, go to sleep
			NIC__TX__DOWN();
		}
...
PointToPointNetDevice::Receive (Ptr<Packet> packet)
{
	...
	if ((m_rxMachineState == RX_READY) || (m_rxMachineState == RX_DOWN))
      ...
      RxMachineState initialState = m_rxMachineState;

			// Calculation of receiving time & emulation
			Time rxTime = Seconds (m_bps.CalculateTxTime (packet->GetSize ()));
			Time rxCompleteTime = rxTime + m_tInterframeGap;

      if (initialState == RX_READY)
        {
          // Correction of the times because the reception is instantaneous
          m_lastRxUpdateTime += rxCompleteTime; // - on RX_READY
          ChangeState (RX_BUSY);
          m_lastRxUpdateTime -= rxCompleteTime; // + on RX_BUSY
        }
      ...
      if (initialState == RX_READY)
		...
		{
			NS_LOG_WARN (m_address << "(" << TxLock << ", " << RxLock << ") [D] " << Simulator::Now ().GetSeconds() << "\t" << " Message received while not RX_READY (" << RX_READY << ") or RX_DOWN (" << RX_DOWN << ") : " << m_rxMachineState);
		}
